...
export class GameSession {
    ...
    handleMessage(msg) {
        switch (msg.type) {
            ...
            case 'player_action':
                if (this.combat) {
                    this.combat.setPlayerDecision(msg.decision);
                }
                break;
            case 'start_campaign':
    ...
    async startCampaign(party, names, difficulty) {
        try {
            this.campaign = CampaignManager.createNew(party, names, difficulty);
            this.campaign.save();
            this.sendCampaignHub();
        }
        catch (err) {
    ...
    async runCampaignBattle() {
        ...
        try {
            ...
            if (result.status === BattleStatus.VICTORY) {
                ...
                if (hasBoss)
                    this.campaign.state.stats.bossesDefeated++;
    ...
    showShop() {
        if (!this.campaign)
            return;
        this.shopItems = ShopSystem.generateInventory(this.campaign.state.currentLevel, this.campaign.state.party.map(c => c.className));
        this.send({
            type: 'shop_data',
    ...
    buyItem(shopIndex, charIndex) {
        if (!this.campaign || shopIndex < 0 || shopIndex >= this.shopItems.length)
            return;
        if (isNaN(charIndex) || charIndex < 0 || charIndex >= this.campaign.state.party.length)
            return;
    ...
    applyTalent(charIndex, talentId) {
        if (!this.campaign)
            return;
        const char = this.campaign.state.party[charIndex];
    ...
    async startPvE(playerId, enemyIds) {
        try {

...
import { Difficulty, BattleStatus, } from '../types/index.js';
export class CampaignManager {
    ...
    static loadFromSave(save) {
        ...
        cm.state.party = save.party.map((sc, idx) => {
            ...
            if (sc.equippedArmor && itemDatabase[sc.equippedArmor])
                char.equippedArmor = new Item(itemDatabase[sc.equippedArmor]);
            cm.state.partyTalents.set(sc.templateId, sc.talents || []);
            return char;
        });
        return cm;
    }
    static createNew(characterIds, names, difficulty) {
        const cm = new CampaignManager();
    ...
    async runBattle(onNeedInput, onBattleState) {
    ...
    checkAchievements() {
        ...
        const newAchievements = AchievementSystem.checkAll(this.state.stats, this.state.achievements, this.state.currentLevel, this.state.difficulty, party);

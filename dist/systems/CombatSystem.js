...
import { BattleAction, BattleStatus, TargetType, CharacterClass, Element, ELEMENT_ADVANTAGE, } from '../types/index.js';
...
export class CombatSystem {
    ...
    criticalHits = 0;
    enemiesDefeated = 0;
    constructor(allies, enemies, playerControlledIds = []) {
        this.allies = [...allies];
        this.enemies = [...enemies];
        this.playerControlledIds = new Set(playerControlledIds);
        for (const char of [...this.allies, ...this.enemies])
            char.resetForBattle();
    }
    log(text, color) {
    ...
    emitState(turnCharId) {
        if (!this.onBattleState)
            return;
        this.onBattleState({
            round: this.round,
    ...
    async waitForPlayerDecision(char) {
        ...
        const baseStat = char.className === CharacterClass.MAGE ? stats.int
            : char.className === CharacterClass.ARCHER ? stats.dex
                : char.className === CharacterClass.DRUID ? stats.int
                    : stats.str;
        const weaponBonus = (() => {
            if (!char.equippedWeapon?.stats)
                return 0;
            if (char.className === CharacterClass.MAGE)
                return char.equippedWeapon.stats.int ?? 0;
            if (char.className === CharacterClass.ARCHER)
                return char.equippedWeapon.stats.dex ?? 0;
            if (char.className === CharacterClass.DRUID)
                return char.equippedWeapon.stats.int ?? 0;
    ...
    executeAttack(attacker, target) {
        ...
        if (!target.isAlive) {
            this.log(`💀 ${target.name} ha sido derrotado!`);
            this.enemiesDefeated++;
        }
    }
    executeAbility(user, target, ability) {
        const stats = user.getEffectiveStats();
        const baseStat = stats[ability.statKey] ?? 0;
        ...
        if (ability.power > 0) {
            ...
            for (const t of targets) {
                ...
                for (let hit = 0; hit < hitCount; hit++) {
                    ...
                    if (!t.isAlive) {
                        this.log(`💀 ${t.name} ha sido derrotado!`);
                        this.enemiesDefeated++;
                    }
                }
            }
        ...
        if (item.healAmount) {
            ...
            for (const [key, val] of Object.entries(item.effect)) {
                ...
                if (typeof val === 'number')
                    target.battleBuffs[k] = (target.battleBuffs[k] || 0) + val;
            }
            this.log(`${user.name} usa ${item.name} obteniendo buff!`);
        }
    }
    executePetAttack(pet, target) {
        const variance = 0.9 + Math.random() * 0.2;
    ...
    processDecision(char, decision) {
        const target = decision.targetId ? this.findChar(decision.targetId) || this.findPet(decision.targetId) : undefined;
        if (decision.targetId && (!target || !target.isAlive))
            return;
        switch (decision.action) {
            case BattleAction.ATTACK: {
                if (target && target.takeDamage)
                    this.executeAttack(char, target);
                break;
            }
            case BattleAction.ABILITY: {
                ...
                if (ability && char.currentStats.mp >= ability.mpCost) {
                    ...
                    if (ability.targetType === TargetType.ALL_ENEMIES) {
                        ...
                        this.executeAbility(char, target || char, ability);
                    }
                    else {
                        if (target)
                            this.executeAbility(char, target, ability);
                    }
                }
                break;
            }
    ...
    getBattleResult() {
        if (this.getAlive(this.enemies).length === 0)
            return BattleStatus.VICTORY;
        if (this.getAlive(this.allies).length === 0 && this.getAlivePets().length === 0)
    ...
    async start() {
        ...
        while (this.round < this.MAX_ROUNDS) {
            ...
            if (status !== BattleStatus.ONGOING)
                return this.finalizeBattle(status);
            this.round++;
            ...
            const allTurnOrder = [
                ...
                const spdA = a.spd || a.getEffectiveStats().spd;
                const spdB = b.spd || b.getEffectiveStats().spd;
                if (spdB !== spdA)
                    return spdB - spdA;
                return Math.random() - 0.5;
            });
            for (const entity of allTurnOrder) {
                if (entity instanceof Character) {
                    ...
                    if (statusCheck !== BattleStatus.ONGOING)
                        return this.finalizeBattle(statusCheck);
                    ...
                    if (this.isPlayerControlled(entity)) {
                        ...
                        const aiDecision = decideAIAction(entity, this.getAlive(this.enemies), this.getAlive(this.allies));
    ...
    processAIDecision(char, aiDecision) {

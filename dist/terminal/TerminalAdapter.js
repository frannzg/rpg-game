...
import { BattleAction } from '../types/index.js';
...
export class TerminalAdapter {
    ...
    async handleNeedInput(info) {
        ...
        switch (action) {
            ...
            case 'ability': {
                ...
                if (!ability)
                    return this.handleNeedInput(info);
                let targetId;
            ...
            case 'item': {
                if (info.availableItems.length === 0)
                    return this.handleNeedInput(info);
                const { itemIdx } = await inquirer.prompt([
                    {
                        type: 'list',
                        name: 'itemIdx',
    ...
    async pickTarget(targets, message) {
        if (targets.length === 0)
            return null;
        if (targets.length === 1)
            return targets[0];
    ...
    async pickAbility(abilityIds, char) {
        ...
        const { abilityId } = await inquirer.prompt([
...
function renderBar(current, max, _color) {
    const len = 12;

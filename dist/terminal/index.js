...
function displayPartyStatus(party) {
    for (const char of party) {
        ...
        if (char.inventory.length > 0)
            console.log(`  🎒 ${char.inventory.map(i => i.name).join(', ')}`);
        if (char.talents.length > 0)
            console.log(`  ⭐ Talentos: ${char.talents.join(', ')}`);
        console.log('');
    }
}
...
async function selectCharacter(prompt) {
...
async function selectEnemies(count) {
    const enemies = [];
    const choices = getCharacterChoices();
...
async function playCampaign() {
    ...
    if (action === 'load') {
        ...
        cm = CampaignManager.createNew(selected.map(c => c.id), names, difficulty);
    ...
    while (cm.state.currentLevel <= 20 && !cm.state.isComplete) {
        ...
        if (isBoss && levelData?.bossName) {
            ...
            if (levelData.bossQuote)
                console.log(pc.italic(pc.gray(`  "${levelData.bossQuote}"`)));
        }
        ...
        for (const char of cm.state.party) {
            ...
            if (availableTalents.length > 0) {
                ...
                const { talentId } = await inquirer.prompt([
                    {
...
async function runShop(cm) {
    showTitle();
    console.log(pc.bold(pc.yellow('\n═══════════ 🏪 TIENDA ═══════════\n')));
    console.log(pc.gray(`Oro: ${cm.state.gold}`));
    const shopItems = ShopSystem.generateInventory(cm.state.currentLevel, cm.state.party.map(c => c.className));
    const { shopAction } = await inquirer.prompt([
        {
    ...
    if (shopAction === 'buy') {
        ...
        const { buyIdx } = await inquirer.prompt([
            { type: 'list', name: 'buyIdx', message: 'Selecciona item:', choices: buyChoices },
        ]);
        if (buyIdx >= 0) {
            const { charIdx } = await inquirer.prompt([
                {
        ...
        if (char.inventory.length === 0) {
            ...
            const { sellIdx } = await inquirer.prompt([
...
async function runInventory(cm) {
    showTitle();
    console.log(pc.bold(pc.yellow('\n═══════════ 🎒 INVENTARIO ═══════════\n')));
    const { charIdx } = await inquirer.prompt([
        {
    ...
    if (action >= 0) {
        ...
        if (item.isEquippable() && item.canEquip(char.className)) {
            ...
            if (use) {
                char.removeFromInventory(action);
                if (item.healAmount) {
                    char.heal(item.healAmount);
                    console.log(pc.green(`❤️ +${item.healAmount} HP!`));
                }
                if (item.mpRestore) {
                    char.restoreMp(item.mpRestore);
                    console.log(pc.blue(`💧 +${item.mpRestore} MP!`));
                }
            }
        }
        else {
...
async function playSurvival() {
    ...
    while (alive) {
        ...
        for (let i = 0; i < enemyCount; i++) {
            ...
            for (const stat of Object.keys(enemy.baseStats)) {
                if (stat === 'maxHp' || stat === 'maxMp')
                    continue;
                enemy.baseStats[stat] = Math.round((enemy.baseStats[stat] || 0) * scale);
            }

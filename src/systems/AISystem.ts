...
import { Ability, TargetType, BattleAction } from '../types/index.js'
...
export function decideAIAction(aiChar: Character, allies: Character[], enemies: Character[]): AIDecision {
  ...
  const healingAbilities = abilities.filter(a =>
    a.targetType === TargetType.SINGLE_ALLY && a.statKey === 'int'
  )
  ...
  if (aiChar.getHpPercent() < 0.3) {
    const healItem = aiChar.inventory.findIndex(
      item => item.healAmount && item.healAmount > 0
    )
    if (healItem !== -1) {
  ...
  const usableAbilities = abilities.filter(
    a => effectiveStats.mp >= a.mpCost && a.targetType !== TargetType.SINGLE_ALLY
  )

import { ItemData, ItemType, EquipSlot, CharacterClass, Rarity, Element } from '../types/index.js'
...
export function getItemsByRarity(rarity: Rarity): ItemData[] {
  return Object.values(itemDatabase).filter(item => item.rarity === rarity)
}

export function getItemsForClass(className: CharacterClass): ItemData[] {
  return Object.values(itemDatabase).filter(item =>
    !item.classRestriction || item.classRestriction.length === 0 || item.classRestriction.includes(className)
  )
}
